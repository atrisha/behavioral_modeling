...
def lambda_op_run(opt_l=None):
    ...
    sum_weights = sum(vel_choice_weights)
    vel_choice_weights = [x/sum_weights for x in vel_choice_weights]
    sample_vel_s_mps = np.random.choice(vel_choice_list,p=vel_choice_weights,size=100000)
    
    sample_range_inverse = np.random.pareto(range_inv_pareto_params,size=100000)
    sample_ttc_inv = []
    count = 0
    ...
    if opt_l is None:
        ...
        for iter in np.arange(max_iters):
            p_crash_iter,q_u_list = simulate_run(opt_l,U_prime,global_crash_prob_dict,wfile,opt_lambda,iter)
            X.append(iter)
            Y.append(p_crash_iter)
...
def sample_vel_s(num_samples):  
    ...
    sum_weights = sum(vel_choice_weights)
    vel_choice_weights = [x/sum_weights for x in vel_choice_weights]
    return np.random.choice(vel_choice_list,p=vel_choice_weights,size=num_samples) 
...
def sample_exp(params,num_samples,range_step_tuple):    
    ...
    return np.random.choice(choice_list,p=ttc_inv_choice_weights,size=num_samples)
...
def cross_entr_sampling(max_iters=100,N_per_iter=1000,sim_with_opt_params = True):
    ...
    if not sim_with_opt_params:
        v_t_final_ttc_inv,v_t_final_range_inv = [],[]
    else:
        v_t_final_ttc_inv = [[0.17110226269764292, 0.23736926], [0.045070486465936595, 0.31582509], [0.0760644074202473, 0.54435407]]
        v_t_final_range_inv = [1.0604453870625663, 0.00955973]
    res_dict = dict()
    res_avg = []
    X,Y = [],[]
    q_u_list = []
    for iter in np.arange(max_iters):
        sample_vel_s_mps_all = sample_vel_s(100000)
        if not sim_with_opt_params:
            sample_range_inv = sample_exp(v_t_minus1_range_inv, 100000,(.007,1,.001))
        else:
            sample_range_inv = sample_exp(v_t_final_range_inv, 100000,(.007,1,.001))
        ...
        for _val in all_configs:
            ...
            if 0 <= _val[0] <15:
                if not sim_with_opt_params:
                    sample_ttc_inv = np.random.exponential(v_t_minus1_ttc_inv[0][0])
                else:
                    sample_ttc_inv = np.random.exponential(v_t_final_ttc_inv[0][0])
                low_speed_configs.append((_val[0],sample_ttc_inv,_val[1]))
            elif 15 <= _val[0] <25:
                if not sim_with_opt_params:
                    sample_ttc_inv = np.random.exponential(v_t_minus1_ttc_inv[1][0])
                else:
                    sample_ttc_inv = np.random.exponential(v_t_final_ttc_inv[1][0])
                med_speed_configs.append((_val[0],sample_ttc_inv,_val[1]))
            else:
                if not sim_with_opt_params:
                    sample_ttc_inv = np.random.exponential(v_t_minus1_ttc_inv[2][0])
                else:
                    sample_ttc_inv = np.random.exponential(v_t_final_ttc_inv[2][0])
                high_speed_configs.append((_val[0],sample_ttc_inv,_val[1]))
        ...
        for idx,all_configs_set in enumerate(all_configs_by_vel):
            ...
            for state_config in all_configs_iter:
                ...
                if has_crashed:
                    ...
                    if sim_with_opt_params:
                        p_u = eval_vel_s(speed_s) * _eval_exp(1/range_x,w_range_inv[0],w_range_inv[1]) * _eval_exp(ttc_inv,w_ttc_inv[idx][0],w_ttc_inv[idx][1])
                        q_u = eval_vel_s(speed_s) * _eval_exp(1/range_x,v_t_final_range_inv[0],v_t_final_range_inv[1]) * _eval_exp(ttc_inv,v_t_final_ttc_inv[idx][0],v_t_final_ttc_inv[idx][1])
            ...
            if not sim_with_opt_params:
                if _val_d_range_inv != 0:
                    v_t_minus1_range_inv[0] = _val_n_range_inv/_val_d_range_inv
                if _val_d_ttc_inv != 0:
                    v_t_minus1_ttc_inv[idx][0] = _val_n_ttc_inv/_val_d_ttc_inv
            total_crash_ct = crash_count + total_crash_ct  
            total_ct = total_ct + n_all_configs
        res_avg.append(total_crash_ct/total_ct)
        if not sim_with_opt_params:
            v_t_final_ttc_inv = v_t_minus1_ttc_inv
            v_t_final_range_inv = v_t_minus1_range_inv
            res_dict[str(iter)] = [(list(v_t_final_ttc_inv),list(v_t_final_range_inv),total_crash_ct/total_ct)]
        if sim_with_opt_params:
    ...
    traci.close() 
    if not sim_with_opt_params:
        print(v_t_final_ttc_inv)
        print('-------')
        print(v_t_final_range_inv)
        print('-------')
        print(res_avg)
